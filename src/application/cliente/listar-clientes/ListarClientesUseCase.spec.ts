...
describe('ListarClientesUseCase', () => {
  ...
  it('retorna clientes cadastrados', async () => {
    const repo = new InMemoryClienteRepository();
    const useCase = new ListarClientesUseCase(repo);
    repo.clientes.push(Cliente.criar({ tipo: TipoCliente.PF, cpfCnpj: '529.982.247-25', nome: 'João', email: 'j@email.com' }));
    repo.clientes.push(Cliente.criar({ tipo: TipoCliente.PJ, cpfCnpj: '11.222.333/0001-81', nome: 'Empresa', email: 'e@email.com' }));
